  ...
  const toggleProgress= () => {
    ...
    const _UploadTostorages = async () => {
        if (_sumitData(name, gmail, phone, password, passwordconfig)) {
            ...
            console.log(avatar);
            let storageRef = storage().ref().child(`images_User/${fileName}.${ext}`);
            const uploadTask = storageRef.putFile(avatar.uri);
    ...
    const addUser = async (result) => {
        toggleProgress()
        const res = await fetch(`${PORT}/api/v1/signup`, {
            method: 'POST',
            headers: {
