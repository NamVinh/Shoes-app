...
const login = ({navigation}) => {
  ...
  const checkLogin = (gmail, password) => {
    if (gmail == '' || password == '') {
  ...
  const loginUser = async () => {
    ...
    if (checkLogin(gmail, password)) {
      setprogressSimple(true)
      const res = await fetch(`${PORT}/api/v1/login`, {
        method: 'POST',
        headers: {
      ...
      console.log(resResult.password + ' kết quả sever trả về ');
      const {data, api_status} = resResult;
      if (api_status === 200) {
